...
Let's separeate this information into the number of reviews (denominator) and
...
glue("There are {sum(num_index)} reviews that fall in products with 10 or more reviews.")
```
...
```{r}
cat("First unhelpful review:\n")
first_neg <- first(which(labels == 0))
text[first_neg]

cat("\nFirst very helpful review:\n")
...
First, you need to convert the labels to an array.

```{r}
labels <- as.array(labels)
```

Next, you need to create your tokenizer. Go ahead and start by using the top
20,000 most frequently used words.
...
Next, convert your text to a numerically encoded sequence.
...
Go ahead and limit the sequences to only use the first 200 words.

```{r}
max_len <- 200
features <- pad_sequences(sequences, maxlen = max_len)

expect_equal(ncol(features), 200)
```
...
Ok, time to build your model architecture and compile it:
...
history <- model %>% fit(
  ...
  callbacks = list(
    ...
    callback_early_stopping(patience = 4, restore_best_weights = TRUE)
    )
)
```
