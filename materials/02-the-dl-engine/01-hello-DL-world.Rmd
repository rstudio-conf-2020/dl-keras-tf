...
We're going to use the _MNIST_ data set which is the "hello world" for learning
deep learning!
...
* use `%<-%` for _object unpacking_ (see `?zeallot::%<-%`)
...
Our current data structure is incompatible for modeling
...
We need to reshape these into a 2D _tensor_
...
```{r optimal-model}
network <- keras_model_sequential() %>%
  layer_dense(units = 512, activation = _____, input_shape = c(28 * 28)) %>%
...
You can predict either the probability of each class. __Note that column 1 is
for digit 0, column 2 is for digit 1, ..., column 10 is for digit 9.
