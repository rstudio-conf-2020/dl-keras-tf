...
We can map the integer values back to the original word index. The integer
number corresponds to the position in the word count list and the name of the
vector is the actual word. 
...
If you a familiar with regularized regression (lasso, ridge, elastic nets 
[ℹ️](https://bradleyboehmke.github.io/HOML/regularized-regression.html)) then
weight regularization is essentially the same thing.
...
discussed here, check out [this notebook]().
