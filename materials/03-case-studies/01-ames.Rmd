...
* Variability in model performance will always exist
...
slides:
  ...
  - feature engineering
  - one-hot encoding
  - importance of data being small, typically btwn 0-1 or standardized
  - illustrate what the final tensor looks like
...
- removing any zero-variance features
...
- feature eng. steps with rsample
...
Now we can now vectorize our training and test data.
...
Remember, batch sizes are typically powers of 2 (i.e. 16, 32, 64, 128, 256).
...
2. add a callback that slowly reduces the learning rate by a factor of 0.1 if 
   we don't experience improvement in our loss for `patience` number of epochs.
...
the same, there will always be some variance in our loss score based on 
