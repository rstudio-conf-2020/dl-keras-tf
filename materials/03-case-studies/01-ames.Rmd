...
2. It appears that we may be underfitting (validation loss is below training loss).
...
- batch sizes commonly take on values of $2^s$ (i.e. 32, 64, 128, 256, 512),
   - smaller batch sizes...
      - can have more variability; however, this variabiliy can help us stay out
        of local minimumns 
...
```{r adj-learning-rate-plot, message=FALSE}
plot(history) + 
  scale_y_log10() +
...
For example, the following model with 3 hidden layers consisting of 1024, 512, 
and 256 units overfits at the minimum validation loss score (but not much) and
shows signs of loss and metric variability. From here, I would start to remove
layers and/or reduce the number of units in each layer until I find a happy
...
approach will be difficult. This is because our validation data (via
`validation_split = 0.2`) only consists of 616 samples. Consequently, model
performance will be highly dependent on these 616 samples.
...
* Callbacks
   - Use early stopping to automate model training and stopping after loss has
     been minimized
   - The optimizer learning rate is a hyperparameter you can and should adjust
